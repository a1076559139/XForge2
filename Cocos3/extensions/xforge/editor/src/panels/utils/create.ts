...
import { Ref } from 'vue';
import { getModuleAssemblyMeta, getModuleAssetBundleMeta } from './meta';
import { getReadme } from './readme';
import { createDirectoryByUrl, isUrlExists } from './utils';

// 创建模块
export async function CreateModule(isGlobal: boolean, moduleName: string = ''): Promise<MessageOptions> {
    ...
    if (!isGlobal) {
        ...
        if (moduleName.toLowerCase() === 'global') {
            return { type: 'warning', message: '名称不能为Global' };
        }

        if (moduleName.toLowerCase() === 'module') {
            return { type: 'warning', message: '名称不能为Module' };
        }

        if (moduleName.toLowerCase().endsWith('global')) {
            return { type: 'warning', message: '名称不能以 Global 结尾' };
        }

        if (moduleName.toLowerCase().endsWith('module')) {
            return { type: 'warning', message: '名称不能以 Module 结尾' };
        }
    }

    const moduleKeyword = !isGlobal ? 'module' : 'global';
    const moduleBundleName = !isGlobal ? `app-module-${moduleName}`.toLowerCase() : 'app-global';
    const moduleFileName = !isGlobal ? `AppModule${moduleName}` : 'AppGlobal';
    const moduleFileUrl = !isGlobal
        ? `db://assets/app-module/${moduleName}`
        : 'db://assets/app-global';

    if (isUrlExists(moduleFileUrl)) {
        return { type: 'warning', message: '模块已存在' };
    }

    let result = await createDirectoryByUrl({
        url: moduleFileUrl,
        readme: getReadme(!isGlobal ? 'module' : 'global'),
        subFolders: [
            {
                name: 'assembly',
                meta: getModuleAssemblyMeta(moduleBundleName)
            },
            {
                name: 'assetbundle',
                meta: getModuleAssetBundleMeta(moduleBundleName, isGlobal)
            }
        ]
    });
