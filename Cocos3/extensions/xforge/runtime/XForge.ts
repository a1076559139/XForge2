...
import { DEBUG, DEV, EDITOR, EDITOR_NOT_IN_PREVIEW } from 'cc/env';
...
export class App {
    ...
    /**
     * 卸载全局模块
     * @param unloadAllLoadedObjects 是否清理资源
     */
    public unloadGlobal(unloadAllLoadedObjects = false) {
        BaseModule.unload(this._global, unloadAllLoadedObjects);
        this._global = null;
    }
    // ###END###
    ...
    public preloadModule(name: string) {
     ...
     * @param unloadAllLoadedObjects 是否清理资源
     */
    public unloadModule(name: string, unloadAllLoadedObjects = false) {
        if (this._moduleCacheMap.has(name)) {
            let module = this._moduleCacheMap.get(name);
            this._moduleCacheMap.delete(name);
            BaseModule.unload(module, unloadAllLoadedObjects);
        }
    }
     ...
     * @param unloadAllLoadedObjects 是否清理资源
     */
    public unloadAllModules(unloadAllLoadedObjects = false) {
