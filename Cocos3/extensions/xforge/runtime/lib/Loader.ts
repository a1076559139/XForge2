import { Asset, AssetManager, Font, ImageAsset, JsonAsset, Label, SceneAsset, Settings, Sprite, SpriteFrame, Texture2D, TextureCube, assetManager, isValid, path, settings, sp } from 'cc';
...
export class Loader {
     ...
     * @param bundle 默认为resources，如果是远程bundle，则使用url末位作为bundle名
     */
    public unloadBundle(bundle?: string, unloadAllLoadedObjects = false) {
        if (!bundle) bundle = 'resources';
    ...
    /**
     * 是否存在Bundle
     * @param bundle 默认为resources，不可以时远程bundle
     */
    public isBundleExist(bundle?: string) {
        if (!bundle) bundle = 'resources';
        const projectBundles = settings.querySettings(Settings.Category.ASSETS, 'projectBundles') as string[];
        return projectBundles.indexOf(bundle) >= 0;
    }
    ...
    public reloadBundle({ bundle, version, onComplete }: { bundle?: string, version?: string, onComplete?: (bundle: AssetManager.Bundle | null) => any }) {
        ...
        assetManager.loadRemote(configUrl, (err: Error, data: JsonAsset) => {
            ...
            this.unloadBundle(path.basename(bundle), true);
